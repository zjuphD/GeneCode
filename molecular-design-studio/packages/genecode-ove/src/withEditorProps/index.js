...
// PATCH (A-OVE-001): do not mutate the store from a selector.
// Upstream wrote `editorState.editorSize = <count>` here, which violates
// Redux immutability and leaks an undeclared `editorSize` key into the
// store ("Unexpected key found in previous state" warning). The count is
// only used as a cutsite-cache busting key in cutsitesSelector, which now
// computes it itself (see selectors/cutsitesSelector.js).
const getEditorState = createSelector(
  state => state.VectorEditor,
  (state, editorName) => editorName,
  (VectorEditor, editorName) => VectorEditor[editorName]
);

// PATCH (A-ENG-002): count the editors that hold sequence data (cache
// capacity for the cutsite results cache — see selectors/cutsitesSelector.js).
function getActiveEditorCount(state) {
  return Object.values(state.VectorEditor || {}).filter(
    editorItem => editorItem && editorItem.sequenceData
  ).length;
}
...
function mapStateToProps(state, ownProps) {
  ...
  const allCutsites = s.cutsitesSelector(
    editorState,
    ownProps.additionalEnzymes,
    // PATCH (A-ENG-002): the cutsite cache capacity is the number of active
    // editors, which only the full store knows (the composed
    // cutsitesSelector itself only receives the per-editor slice).
    getActiveEditorCount(state)
  );
