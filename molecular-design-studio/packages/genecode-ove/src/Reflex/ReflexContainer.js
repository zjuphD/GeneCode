...
const getDomNode = component =>
  component && typeof component.getDomNode === "function"
...
class ReflexContainer extends React.Component {
  ...
  constructor(props) {
    ...
    // A-ENG-001: React 18 dropped string refs; keep guid -> instance/DOM node
    // callbacks here instead of relying on `this.refs[guid]`.
    this.refMap = {};
  }
  ...
  /////////////////////////////////////////////////////////
  getSize(element) {
    const ref = element.ref ? this.refMap[element.ref] : element;

    const domElement = getDomNode(ref);
  ...
  emitElementsEvent(elements, event) {
    this.toArray(elements).forEach(element => {
      if (element.props[event]) {
        const ref = this.refMap[element.ref];

        element.props[event]({
          domElement: getDomNode(ref),
          component: element
        });
      }
    });
    ...
    this.children = React.Children.map(
      ...
      (child, idx) => {
        ...
        const newProps = Object.assign({}, child.props, {
          ...
          ref: el => {
            // prune on unmount (callback refs receive null) so the map stays bounded
            if (el) this.refMap[flexData.guid] = el;
            else delete this.refMap[flexData.guid];
          },
          index: idx
        });
