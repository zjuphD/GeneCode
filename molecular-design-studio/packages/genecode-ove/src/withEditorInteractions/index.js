...
function VectorInteractionHOC(Component /* options */) {
  return class VectorInteractionWrapper extends React.Component {
    ...
    cutsiteDoubleClicked = ({ annotation }) => {
      const isoschizomerNames = annotation?.isoschizomerNames;
      if (Array.isArray(isoschizomerNames) && isoschizomerNames.length > 1) {
        // Merged cut-site label (Sequence view): open a dialog listing every
        // isoschizomer enzyme at this position instead of one enzyme's info.
        showDialog({
          dialogType: "IsoschizomerCutsitesDialog",
          props: {
            enzymeNames: isoschizomerNames
          }
        });
        return;
      }
