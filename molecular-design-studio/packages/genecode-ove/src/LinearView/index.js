...
class _LinearView extends React.Component {
  state = {};
  // React 18: pass the actual DOM node to react-draggable instead of letting
  // it fall back to the deprecated ReactDOM.findDOMNode API.
  dragNodeRef = React.createRef();
  ...
  render() {
    ...
    // Reflex can briefly report a negative width while the editor shell is
    // opening/closing a side panel. Do not propagate that transient measurement
    // into SVG width attributes (the browser rejects negative values).
    const safeWidth = Math.max(Number(width) || 0, 0);
    let innerWidth = Math.max(safeWidth - marginWidth - 20, 0);
    if (isNaN(innerWidth)) {
      innerWidth = 0;
    }
          ...
          <PinchHelperToUse {...(linearZoomEnabled && pinchHandler)}>
            ...
            <RowItem
              {...{
                ...
                showAminoAcidUnitAsCodon,
                onScroll: () => {
                  this.easyStore.viewportWidth = safeWidth;
                  const row =
