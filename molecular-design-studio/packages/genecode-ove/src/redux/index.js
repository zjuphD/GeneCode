...
const reducers = {
  ...
  // PATCH (A-ENG-002): register the cutsite-cache capacity key. Upstream
  // used to mutate `editorSize` onto the editor slice from a selector (the
  // mutation was removed in A-OVE-001) and standard combineReducers would
  // warn about + drop the undeclared key, so the cache always fell back to
  // capacity 1. Nothing writes the key to the store anymore — the capacity
  // is threaded through as a selector argument — but registering this
  // pass-through means combineReducers can never warn about an
  // `editorSize` key again if a legacy path reintroduces it.
  editorSize: (state = null) => state,
  instantiated: () => true
};
