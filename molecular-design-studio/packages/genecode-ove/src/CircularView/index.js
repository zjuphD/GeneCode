...
export function CircularView(props) {
  ...
  const layersToDraw = [
      ...
      : []),
    {
      zIndex: 10,
      layerName: "cutsites",
      // A-UI-001: upright enzyme labels (upstream ships red italic); the
      // Sequence panel already renders them upright + semibold, so the Map
      // view now shares the same label language.
      fontStyle: "normal",
      Annotation: Cutsite,
  ...
  return (
    ...
    >
      ...
      <Draggable
        // React 18: react-draggable otherwise falls back to findDOMNode.
        nodeRef={target}
