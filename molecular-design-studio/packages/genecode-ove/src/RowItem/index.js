...
export default function RowItem(props) {
  ...
  if (isLinearView) {
    // A transient zero-width measurement is possible while Reflex is laying
    // out the editor. The linear-view label gutter used to subtract from that
    // zero and emit a negative SVG width (React logs an error and browsers
    // discard the axis). Keep the measurement fail-safe at zero.
    width = Math.max(
      0,
